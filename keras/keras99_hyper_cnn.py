...
#2. 모델(cnn-마지막 output에서 flatten쓰고 dense로 바꿔주기, input에서는 flatten하지 않는다)
...
def build_model(drop=0.5, optimizer='adam'):
    ...
    x = Conv2D(512,(3,3),activation='relu', name='hidden1',padding="same")(inputs)
    x = Dropout(drop)(x)
    x = Conv2D(256,(2,2),activation='relu',name='hidden2',padding="same")(x)
    x = Dropout(drop)(x)
