...
# model.add(Dense(10,activation='softmax')) #레이어 개수 추가하면 오류가 난다.(load_weights에서)
...
#weight값을 불러오겠다
#model만 save하면 데이터 없어도 됨
#그러나 가중치만 save하려면 데이터 있어야 함

model.load_weights('./model/test_weight1.h5') 
#컴파일 주석 처리하면 돌아가지 않는다. #순수하게 weight만 가져온다.#fit만 주석처리하면 돌아간다. (엄청 빨리 돌아간다)
