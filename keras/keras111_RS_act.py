...
# Dense모델 구성
# 모델을 여러번 쓸 수 있다
def build_model(drop, optimizer, learning_rate, activation): # 초기값 넣거나 변수명만 넣어도 상관없다.
...
def create_hyperparameters():
    ...
    dropout = list(np.linspace(0.1, 0.5, 5)) #0.1부터 0.5까지 5등분
    activation = ['sigmoid','relu','elu','tanh',leaky_relu]
    epoch = [10,15]
    splt = [0.1,0.2]
    # epoch도 넣을 수 있고, node의 개수, activation도 넣을 수 있다(많이 넣을 수 있다.)
    # activation의 sigmoid와 softmax는 주의할 것
    return{"batch_size": batches, "optimizer" : optimizer, "drop" : dropout, "learning_rate" : learning_rate, "activation": activation,"epochs":epoch,"validation_split":splt}
...
#결과
"""
{'optimizer': <class 'keras.optimizers.Nadam'>, 'learning_rate': 0.25, 'drop': 0.5, 'batch_size': 128, 'activation': <keras.layers.advanced_activations.LeakyReLU object at 0x0000022C68DD7408>}
"""
