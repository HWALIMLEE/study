...
# pad_x = pad_sequences(x, padding="post",value=1.5)
print(pad_x)   # (12, 5)
...
model = Sequential()
# model.add(Embedding(word_size, 10)) # 이것만 기억!/ 와꾸 맞춰주는 부분 (25, 임의의 수(히든), 5)=(전체 단어의 개수, 아웃풋, 5)<=== (12,5)(행 무시) #(None, 5, 10)=(None, input_length, output)
# 삼차원 == LSTM, Conv1D
...
# Embedding과 LSTM 같이 쓰면 input_shape 안써줘도 됨
...
# x에 대한 input을 주지 않은 것인데 shape이 틀렸다는 오류가 나온다. 
model.add(Conv1D(50,1,input_shape=(5,1)))
# (4*(input+output+1)*3)==(4*(10+3+1)*3)
...
# 문장 내 단어의 순서는 시계열과 일맥상통
# nan의 자료형은 float
# embedding에서는 LSTM 제일 많이 쓴다. 
# embedding 안 쓰고 LSTM으로도 가능