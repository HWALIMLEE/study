...
model.add(Dense(3)) #output 을 바꿈 keras45파일에서 오류난다. 
model.add(Dense(1, name='new')) #output 을 바꿈 keras45파일에서 오류난다. 
model.summary() 
...
hist=model.fit(x,y,epochs=100,batch_size=1,callbacks=[early_stopping,tb_hist],validation_split=0.2) #model.fit을 변수에 할당해준다. 
...
# tensorboard란 웹 서버를 실행하는 것
# 내 pc의 6006번 포트를 사용하면 tensorboad를 웹으로 연다.-->공모전 나갈때 제출하면 good


#cmd 창에
#graph경로 적어주고
#tensorboard --logdir=.
#아이피 주소: 127.0.0.1:6006--->들어가서 확인
